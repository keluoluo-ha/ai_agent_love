...
@Slf4j
public class YuManusAgent extends ToolCallAgent {

    public YuManusAgent(ToolCallback[] alltools, ChatModel dashscopeChatModel) {
        super(alltools);
        ...
        String nextStepPrompt = """
                Workflow rules:
                0. 【最高优先级】需要向用户提问或索取信息时，必须调用 askHuman 工具；禁止只在 message 文本里列出问题而不调用工具。
                ...
                7. When key information is missing, ambiguous, or needs user confirmation, call askHuman immediately with one consolidated Chinese question (and optional options). Do NOT ask for information already provided.
                8. After askHuman returns the user's answer, continue the task with that information.
                """;
        this.setSystemPrompt(systemPrompt);
        this.setNextPrompt(nextStepPrompt);
        this.setMaxSteps(20);
